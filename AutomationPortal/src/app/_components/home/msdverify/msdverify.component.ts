...
@Component({
  selector: 'app-msdverify',
  templateUrl: './msdverify.component.html',
  styleUrls: ['./msdverify.component.scss']
})
export class MsdverifyComponent implements OnInit {
  jsonStatus: any = [];
  jsonStatus1: any = [];
  request: object = {};
  FromDate: string = '';
  ...
  setStatusData() {
    this.loaderService.startLoading();
    this.snq.getMSDStatusSummary(1)
      .subscribe((responseData: ResponseData) => {
        this.loaderService.stopLoading();
  ...
  setGroupFliter(request) {
    this.loaderService.startLoading();
    this.snq.msdverifytaskEnquiryList(this.request)
      .pipe()
      .subscribe(
        data => {
          this.loaderService.stopLoading();
