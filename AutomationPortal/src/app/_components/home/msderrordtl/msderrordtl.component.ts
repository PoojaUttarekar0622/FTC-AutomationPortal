...
@Component({
  selector: 'app-msderrordtl',
  templateUrl: './msderrordtl.component.html',
  styleUrls: ['./msderrordtl.component.scss']
})
export class MsderrordtlComponent implements OnInit {
  ...
  setStatusData(pkId) {
    ...
    this.snq.getMSDDetails(pkId)
      .subscribe((responseData: ResponseData) => {
        ...
        if (this.jsonHeader.quotationNo === "" || this.jsonHeader.quotationNo === null) {
          this.showdeletebtn = true;
        } else {
          this.showdeletebtn = false;
        }
        this.hdrleadTimeforitem = this.jsonHeader.leadTimeforitem;
        this.leadTimeperiod = this.jsonHeader.leadTimeperiod;
      },
        err => {
          this.loaderService.stopLoading();
          console.log(err);
  ...
  addNewRow() {
    if (this.jsonHeader.itemDetails == null) {
      this.jsonHeader.itemDetails = [{
      ...
      this.jsonHeader.itemDetails.push({
        "pK_MSDENQUIRY_DTL_ID": 0,
        "fkEnquiryid":this.hdrId,
        "partCode": "",
        "partName": "",
  ...
  btnUpdateData() {
    ...
    for (let i = 0; i < detailsArray.length; i++) {
      ...
      if (detailsArray[i]['seqNo'] == "") {
        ...
        valid = false;
        break;
      }  else if (detailsArray[i]['leadTime'] && detailsArray[i]['leadTime'] !== "" && !detailsArray[i]['leadTimeperiod'] && detailsArray[i]['leadTimeperiod'] != "") {
        this.toastr.show(
        ...
        valid = false;
        break;
      } else if (detailsArray[i]['leadTimeperiod'] && detailsArray[i]['leadTimeperiod'] != "" && !detailsArray[i]['leadTime'] && (detailsArray[i]['leadTime'] == "" || detailsArray[i]['leadTime'] == null)) {
        this.toastr.show(
  ...
  saveAsDraft() {
    this.loaderService.startLoading();
    this.jsonHeader["status"] = "5";
    this.jsonHeader["saveAsDraft"] = "saveInError";
    let details: any[] = this.jsonHeader["itemDetails"];
    if (details) {
      details.forEach(function (item) {
        item["status"] = "5";
