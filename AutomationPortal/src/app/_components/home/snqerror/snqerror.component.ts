...
@Component({
  selector: 'app-snqerror',
  templateUrl: './snqerror.component.html',
  styleUrls: ['./snqerror.component.scss']
})
export class SnqerrorComponent implements OnInit {
  jsonStatus: any = [];
  jsonStatus1: any = [];
  request: object = {};
  FromDate: string = '';
    ...
    this.snq.getStatusCountSNQ(this.request)
      ...
      .subscribe(
        data => {
          ...
          console.log("jsoncount", this.jsonStatus1);
        },
        error => {
        });
  }
  ...
  setGroupFliter(request) {
    this.loaderService.startLoading();
    this.snq.snqerrortaskEnquiryList(this.request)
      .pipe()
      .subscribe(
        data => {
          this.loaderService.stopLoading();
